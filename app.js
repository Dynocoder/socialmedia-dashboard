const cardContainer = document.getElementById('social-cards');

console.log(cardContainer);

let socialat = ["@nathanf", "@nathanf", "@realnathanf", "Nathan F."];
let socialcount = ["1987", "2344", "11k", "3422"];
let socialSymbol = [
...
let up = '<svg xmlns="http://www.w3.org/2000/svg" width="8" height="4"><path fill="#1EB589" fill-rule="evenodd" d="M0 4l4-4 4 4z"/></svg>';
let down = '<svg xmlns="http://www.w3.org/2000/svg" width="8" height="4"><path fill="#DC414C" fill-rule="evenodd" d="M0 0l4 4 4-4z"/></svg>';
...
for (var i = 0; i < 4; i++) {
    ...
    name.innerHTML = socialSymbol[i];
    let nameP = document.createElement('p')
    nameP.innerText = socialat[i];
    ...
    let sCount = document.createElement('h1')
    sCount.innerText = socialcount[i]
    cf.appendChild(sCount)
    if (i < 3) {
        ...
        var p = document.createElement('p')
        p.innerText = "SUBSCRIBERS"
        cf.appendChild(p)
    }
    cardText.appendChild(cf);
    ...
    if (i < 3) {
        ...
        change.innerHTML = down;
    }
    
    let todayText = document.createElement('p');
    todayText.innerText = 'Today';
    change.appendChild(todayText)
    

    cardText.appendChild(change);
...
function toggleDarkMode() {
    ...
    let vs = document.querySelector(":root");
    if (!darkOn) {
        darkOn = true;
        vs.style.setProperty('--card-bg', 'hsl(228, 29%, 20%)')
        vs.style.setProperty('--body-bg', 'hsl(225, 17%, 14%)')
        vs.style.setProperty('--heading-clr', 'hsl(0, 0%, 100%)')
        vs.style.setProperty('--card-bg-active', 'hsl(228, 26%, 27%)')
    } else {
        darkOn = false;
        vs.style.setProperty('--card-bg', 'hsl(227, 47%, 96%)')
        vs.style.setProperty('--body-bg', 'hsl(0, 0%, 100%)')
        vs.style.setProperty('--heading-clr', 'hsl(230, 17%, 14%)')
        vs.style.setProperty('--card-bg-active', 'hsl(232, 33%, 91%)')
